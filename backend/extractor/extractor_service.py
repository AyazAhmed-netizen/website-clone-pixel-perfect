...
# ============================================
# Synchronous Playwright fallback for Windows
# ============================================
# On Windows with Python 3.13, async Playwright fails because
# asyncio.create_subprocess_exec raises NotImplementedError.
# This sync fallback runs Playwright in its own thread.

def _extract_sync(url: str, viewport_width: int = 1920, viewport_height: int = 1080) -> dict:
    """Synchronous Playwright extraction (runs in a thread)
    
    This function runs in a dedicated thread via asyncio.to_thread()
    to avoid blocking the event loop while providing Playwright
    functionality on Windows where async Playwright fails.
    """
    from playwright.sync_api import sync_playwright
    
    logger.info(f"[SyncExtract] Starting sync extraction for: {url}")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                css_content = styles or ''
            except Exception:
                pass
            
            logger.info(f"[SyncExtract] Done. Title: {title}, HTML: {len(html)} chars")
