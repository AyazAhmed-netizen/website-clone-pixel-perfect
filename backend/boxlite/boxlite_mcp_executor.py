...
class BoxLiteMCPExecutor:
    ...
    async def _execute_crawl_website(self, input: Dict[str, Any]) -> tuple:
        """Crawl and extract a website URL using Playwright
        
        Uses async Playwright first, falls back to sync Playwright on Windows
        where asyncio.create_subprocess_exec raises NotImplementedError.
        """
        url = input.get("url", "")
        viewport_width = input.get("viewport_width", 1920)
        viewport_height = input.get("viewport_height", 1080)
        ...
        # Try async Playwright first, fall back to sync on Windows
        try:
            logger.info(f"[Crawl] Starting extraction for: {url}")

            from extractor.extractor_service import PlaywrightExtractorService
            from extractor.models import ExtractRequest
            from extractor.cache_manager import extraction_cache
            ...
            result = await extractor.extract_quick(request)

            if not result.success:
                raise Exception(f"Async extraction failed: {result.error}")

            request_id = result.request_id
            logger.info(f"[Crawl] Quick extraction done, waiting for DOM phase...")
            ...
            while waited < max_wait:
                ...
                waited += 2
                cache_entry = await extraction_cache.get(request_id)
                if cache_entry and cache_entry.dom_tree is not None:
                    logger.info(f"[Crawl] DOM phase complete after {waited}s")
                    break

            if cache_entry is None:
                raise Exception("Failed to get extraction results after waiting")

            source_id = request_id
            page_title = cache_entry.title or "Untitled"
            metadata = cache_entry.metadata
            element_count = metadata.total_elements if metadata else "N/A"

            available = []
            if cache_entry.dom_tree:
                available.append("dom_tree")
            if cache_entry.style_summary:
                available.append("style_summary")
            if cache_entry.screenshot:
                available.append("screenshot")
            if cache_entry.assets:
                available.append("assets")
            if cache_entry.css_data:
                available.append("css_data")
            if cache_entry.components:
                available.append("components")

            result_text = (
                f"\u2705 Website crawled successfully!\n\n"
                ...
                f"**Next Steps:**\n"
                f"1. Call get_layout(source_id=\"{source_id}\") to analyze page sections\n"
                f"2. Call spawn_section_workers(sections=[...]) to generate components\n"
                f"3. Write App.jsx and index.css to integrate everything\n\n"
                f"\U0001f4a1 You can now proceed with cloning this website!"
            )
            return (result_text, False)

        except Exception as async_err:
            logger.warning(f"[Crawl] Async extraction failed ({async_err}), trying sync fallback...")

        # ============================================
        # FALLBACK: Use synchronous Playwright (works on Windows Python 3.13)
        # ============================================
        try:
            from extractor.extractor_service import _extract_sync

            sync_result = await asyncio.to_thread(
                _extract_sync, url, viewport_width, viewport_height
            )

            if not sync_result.get('success'):
                return (f"Error: Both async and sync extraction failed for {url}", True)

            # Store extracted data in cache for get_layout() to use later
            from extractor.cache_manager import extraction_cache
            from cache.memory_store import extraction_cache as mem_cache
            import hashlib

            source_id = f"sync-{hashlib.md5(url.encode()).hexdigest()[:12]}"
            page_title = sync_result.get('title', 'Untitled')
            html_content = sync_result.get('html', '')
            text_content = sync_result.get('text', '')
            css_content = sync_result.get('css', '')
            screenshot_b64 = sync_result.get('screenshot')

            # Store in memory cache for get_layout
            entry_id = mem_cache.store(
                url=url,
                data={
                    'url': url,
                    'title': page_title,
                    ...
                    'source_type': 'sync_extraction',
                },
                title=page_title,
            )
            # Use the returned entry_id as source_id if different
            if entry_id:
                source_id = entry_id

            result_text = (
                f"\u2705 Website crawled successfully (sync mode)!\n\n"
                f"**Source ID:** {source_id}\n"
                f"**Page Title:** {page_title}\n"
                f"**URL:** {url}\n"
                ...
                f"**Data extracted via sync Playwright.**\n"
                f"You now have the full HTML, CSS, and text content of this page.\n"
                f"Build the clone directly using this data!\n\n"
                f"**Next Steps:**\n"
                f"1. Use the extracted HTML/CSS to build the clone\n"
                f"2. Write App.jsx with the page structure\n"
                f"3. Write index.css with the styles"
            )
            return (result_text, False)

        except Exception as sync_err:
            logger.error(f"[Crawl] Sync extraction also failed: {sync_err}", exc_info=True)
            return (f"Error crawling website: {str(sync_err)}", True)

    # Commands that are known to hang or are dangerous on Windows
    BLOCKED_COMMANDS = [
        (r'npx\s+kill-port', 'Use the built-in port management instead of npx kill-port'),
        (r'kill-port', 'Use the built-in port management instead of kill-port'),
        (r'\bsleep\b', 'Sleep commands are not needed - the system handles delays automatically'),
        (r'kill\s+\$\(lsof', 'lsof does not exist on Windows. Use the built-in port management.'),
    ]

    # Patterns to auto-rewrite for Windows compatibility (search sandbox dir instead of whole filesystem)
    REWRITE_PATTERNS = [
        (r'\bfind\s+/\s', 'find /'),  # find / -> find . (search current dir, not all drives)
    ]

    # Max timeout for foreground commands (prevents infinite hangs)
    MAX_SHELL_TIMEOUT = 120  # seconds (increased for Git Bash --login startup time on Windows)

    async def _execute_shell(self, input: Dict[str, Any]) -> tuple:
        """Execute shell command"""
        import re as _re
        command = input.get("command", "")
        timeout = min(input.get("timeout", 60), self.MAX_SHELL_TIMEOUT)
        ...
        for pattern, reason in self.BLOCKED_COMMANDS:
            if _re.search(pattern, command):
                logger.warning(f"[Shell] Blocked command: {command[:80]} ({reason})")
                # If it's a kill-port command, redirect to built-in
                if 'kill-port' in command and 'npx' in command:
                    # Extract port number if present
                    port_match = _re.search(r'(\d{4,5})', command)
                    if port_match:
                        port = int(port_match.group(1))
                        logger.info(f"[Shell] Redirecting kill-port to built-in _kill_port for port {port}")
                        success = await self.sandbox._kill_port(port)
                        return (f"Port {port} killed using built-in method (success={success})", False)
                return (
                    f"Error: This command is blocked: {reason}\n"
                    f"Command was: {command[:100]}",
                    True
                )

        # ============================================
        # Auto-rewrite commands for Windows compatibility
        # ============================================
        if _IS_WINDOWS:
            # find / -> find . (on Windows Git Bash, find / searches ALL drives, hanging for minutes)
            if _re.search(r'\bfind\s+/\s', command):
                command = _re.sub(r'\bfind\s+/\s', 'find . ', command, count=1)
                logger.info(f"[Shell] Rewrote 'find /' to 'find .' for Windows compatibility: {command[:80]}")

        # ============================================
        # Dev server command check
        # ============================================
        ...
        if is_dev_server_cmd:
            ...
            if state.preview_url:
                return (
                    f"Error: Dev server is already running!\n"
                    f"Preview URL: {state.preview_url}\n"
                    f"Use get_build_errors() to check for errors, or view the preview.",
                    True
                )
            ...
            for t in running_terminals:
                if any(cmd in (t.command or "") for cmd in dev_server_commands):
                    return (
                        f"Error: Dev server process already exists!\n"
                        f"Terminal: {t.name} (ID: {t.id})\n"
                        f"Command: {t.command}\n"
                        f"Wait for preview_url to be ready, or use diagnose_preview_state().",
                        True
                    )

        # ============================================
        # Execute command with timeout cap
        # ============================================
        logger.info(f"[Shell] Executing: {command[:80]} (timeout={timeout}s, bg={background})")
        ...
        if result.success:
            ...
            # Add helpful hint for timeout errors
            if "timed out" in error.lower() or "timeout" in error.lower():
                error += f"\nHint: Command exceeded {timeout}s timeout. Try breaking it into smaller steps."
