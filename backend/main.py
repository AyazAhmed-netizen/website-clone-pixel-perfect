...
# LLM module (provider/model listing + credential test)
try:
    from agent.llm_routes import router as llm_router
    app.include_router(llm_router)
    logger.info("Registered: /api/llm/*")
    ...
    try:
        if sys.platform == "win32":
            ...
            for line in result.stdout.splitlines():
                ...
                if len(parts) > 1 and f":{port}" in parts[1]:
                    try:
                        pids.add(parts[-1])
...
async def startup_event():
    ...
    port_for_log = os.getenv("PORT", "5001")
